...
### 4.2 Production Model Strategy

The uploaded contest data affects the model choices:

* `Document_config_data` contains hundreds of PDF files with a mostly usable text layer, so OCR should not be the default extraction path.
* `API_config_data` contains a compact API registry of roughly one hundred API candidates, so high-quality reranking is practical.
* `example_data` is small but balanced between `call_document` and `call_api`, so the selector should stay lightweight and carefully calibrated.
* `test_data` contains both multiple-choice document questions and API configuration questions, so routing must remain based on `question` only.

The system therefore uses strong pretrained models where they improve accuracy, while keeping schema-sensitive steps deterministic.

#### A. PDF Parsing, OCR, and Layout Understanding

Primary document extraction should use text-layer parsers first:

* `PyMuPDF` or `pdfplumber` for text blocks, page numbers, and basic layout
* `pypdf` as a lightweight fallback when only plain text extraction is needed

OCR is used only as an offline fallback for pages with poor text extraction, scanned images, screenshots, or text embedded inside figures:

* primary OCR fallback: `PaddleOCR PP-OCRv5`
* layout, table, formula, and reading-order recovery: `PaddleOCR PP-StructureV3`

This keeps inference fast and avoids introducing OCR noise into pages that already contain reliable text. When time allows, `PP-StructureV3` can be run offline on all pages that contain tables, formulas, columns, or dense visual layout, then exported into retrieval-friendly Markdown.

#### B. Embedding Model

The accuracy-first embedding model is:
...
* PDF chunk retrieval
* API candidate retrieval
* semantic alias matching when exact and fuzzy matching do not resolve a value

Recommended query instruction examples:

* document retrieval: `Given a Vietnamese technical question, retrieve the most relevant PDF passages that answer it.`
* API retrieval: `Given a Vietnamese dashboard/API question, retrieve the API specification that should be called.`

Recommended production settings:

* precompute all document and API embeddings offline
* normalize vectors and use cosine or inner-product search consistently
* keep `doc_id`, `page`, heading, table id, and chunk type as metadata
* use shorter chunks for tables and parameter descriptions, and medium chunks for narrative PDF text

#### C. Hybrid Retrieval

Dense embeddings alone are not enough for this data because many questions contain exact identifiers such as:

* `Public_441`, `Public_659`
* API metric names like `SLSX`, `SLNT`, `RA`, `leakage rate`
* organization aliases like `TTPMVT`, `TTPMQT`, `TTPMTCS`
* backend values such as `presales`, `odc/osdc`, `package`

The first-stage retriever should therefore combine:

* lexical retrieval: BM25 or BGE-M3 sparse lexical weights
* dense retrieval: `Qwen/Qwen3-Embedding-8B`
* metadata filters when the question explicitly names a PDF or other stable identifier

`BAAI/bge-m3` remains a strong backup or ablation model because it supports dense, sparse, and multi-vector retrieval in one model. It is useful if deployment simplicity matters more than maximum accuracy.

#### D. Reranking Model

The accuracy-first reranker is:

* `Qwen/Qwen3-Reranker-8B`

Use it after first-stage retrieval:

* document branch: rerank top 50-100 PDF chunks
* API branch: rerank top 30-50 API candidates, or all API candidates because the registry is small
* option scoring: rerank evidence for each `question + option` pair

`BAAI/bge-reranker-v2-m3` is the practical backup if lower latency or easier serving is required.

#### E. LLM Usage

The LLM should not replace deterministic retrieval, routing, alias normalization, or schema validation. It is used only for controlled fallback decisions:

* resolving ambiguous API slot extraction after exact, fuzzy, and embedding matching
* judging close multiple-choice options when reranker scores are too close
* repairing a candidate JSON only when it can be validated against the selected API schema

The preferred local accuracy-first LLM is:

* `Qwen/Qwen3-32B`

Production serving should use `vLLM` or `SGLang` with:

* JSON schema, regex, or choice-constrained decoding where possible
* low output token limits for slot extraction and option verification
* temperature near zero for schema tasks, or deterministic constrained decoding
* strict post-generation validation before accepting any output

For API generation, the LLM output must never be trusted directly. The final answer is produced by:

1. selecting a known API template
2. filling only schema-approved fields
3. normalizing values against alias dictionaries and allowed enums
4. validating the final JSON body

#### F. Selector Model

The branch selector should remain lightweight and auditable:

* `TF-IDF` word and character n-grams
* `LogisticRegression`, `LinearSVC`, or a calibrated shallow classifier
...
The recommended stack for this contest data is:

* document parsing: `PyMuPDF` / `pdfplumber`
* OCR fallback: `PaddleOCR PP-OCRv5`
* layout/table/formula recovery: `PaddleOCR PP-StructureV3`
* embeddings: `Qwen/Qwen3-Embedding-8B`
* reranker: `Qwen/Qwen3-Reranker-8B`
* LLM fallback/verifier: `Qwen/Qwen3-32B`
* serving: `vLLM` or `SGLang`
* deterministic layers: date parser, alias dictionary, fuzzy matching, template filling, JSON schema validation
...
* `Qwen/Qwen3-Embedding-8B` for dense retrieval
* `Qwen/Qwen3-Reranker-8B` for final candidate reranking
* `Qwen/Qwen3-32B` for controlled fallback verification
* `BAAI/bge-m3` and `BAAI/bge-reranker-v2-m3` as practical backup models
* OCR/layout model settings for `PaddleOCR PP-OCRv5` and `PP-StructureV3`
...
Extracts text blocks and page metadata from PDF documents using text-layer parsing as the primary path.

#### `ocr_parser.py`

Handles OCR for low-text, scanned, screenshot-heavy, or image-heavy pages using `PaddleOCR PP-OCRv5`.

#### `table_parser.py`

Extracts or linearizes tables, formulas, and layout-aware reading order into retrieval-friendly text, preferably using `PP-StructureV3` offline.
...
Searches relevant document chunks for a given question using hybrid lexical + dense retrieval.
...
Scores candidate options against retrieved document evidence and can call the reranker on each `question + option` pair.
...
Retrieves top API candidates from the API registry using hybrid lexical + dense retrieval.

#### `api_reranker.py`

Reranks top candidate APIs for more accurate selection, using `Qwen/Qwen3-Reranker-8B` in the accuracy-first setup.

#### `slot_extractor.py`

Extracts required entities and parameter values from the question with deterministic rules first and LLM fallback only for unresolved ambiguity.
...
Wrapper for the embedding model used in document retrieval, API retrieval, and semantic alias matching.

#### `reranker_model.py`

Wrapper for the reranking model used in API candidate selection, PDF evidence reranking, and option scoring.

#### `llm_wrapper.py`

Lightweight wrapper around the local LLM when constrained JSON extraction, ambiguous slot resolution, or close-option verification is needed.
...
* benchmark `Qwen/Qwen3-Embedding-8B` against `BAAI/bge-m3` on local document/API retrieval accuracy
* benchmark `Qwen/Qwen3-Reranker-8B` against `BAAI/bge-reranker-v2-m3` for difficult API selection cases
* better OCR and layout fallback for formula-heavy PDFs using `PP-OCRv5` and `PP-StructureV3`
